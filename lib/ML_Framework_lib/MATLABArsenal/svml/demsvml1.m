...
% $Revision: 1.2 $ $Date: 2002/02/19 12:28:55 $
...
[Y, X] = svmlread('D:/Research/svm_light_windows/example1/train.dat');
net = svml('D:/Research/svm_light_windows/example1/model1', 'Kernel', 0, 'C', 1);
net = svmltrain(net, X, Y);
% Also read out Joachims' test data
[Ytest, Xtest] = svmlread('D:/Research/svm_light_windows/example1/test.dat');
...
net = svml('D:/Research/svm_light_windows/example1/model2', 'Kernel', 0, 'C', 1);
% directly access training data
net = svmltrain(net, 'D:/Research/svm_light_windows/example1/train.dat');
% Compute prediction on the test data
Ytest = svmlfwd(net, 'D:/Research/svm_light_windows/example1/test.dat');


% Now for the transductive SVM:
net = svml('D:/Research/svm_light_windows/example2/model1', 'Kernel', 0, 'C', 1, 'TransLabelFile', ...
           'D:/Research/svm_light_windows/example2/trans_predictions');
...
net = svmltrain(net, 'D:/Research/svm_light_windows/example2/train_transduction.dat');
% Read out the labels for the unclassified training examples
transLabels = svmlread('D:/Research/svm_light_windows/example2/trans_predictions');
...
options = svmlopt('ExecPath', 'D:/Research/svm_light_windows');
