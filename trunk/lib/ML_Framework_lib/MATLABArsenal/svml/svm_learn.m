...
% $Revision: 1.5 $ $Date: 2002/02/19 12:25:18 $
...
if (isempty(options.ExecPath))
      options.ExecPath = '';
end;
evalstr = ['!' fullfile(options.ExecPath, 'svm_learn -v 0 ') s ' ' ...
           examples ' ' model];
%evalstr = sprintf('!svm_learn %s %s %s', s, ...
%           examples, model);
fprintf('Calling SVMlight:%s\n', evalstr);
eval(evalstr);
status = 0;
