function [KResult,k]= kappa1(varargin)
...
%display results
Results = [
sprintf('Observed agreement (po) = %0.4f\n',po)...
sprintf('Random agreement (pe) = %0.4f\n',pe)...
sprintf('Agreement due to true concordance (po-pe) = %0.4f\n',po-pe)...
sprintf('Residual not random agreement (1-pe) = %0.4f\n',1-pe)...
sprintf('Cohen''s kappa = %0.4f\n',k)...
sprintf('kappa error = %0.4f\n',sek)...
sprintf('kappa C.I. (alpha = %0.4f) = %0.4f     %0.4f\n',alpha,ci)...
sprintf('Maximum possible kappa, given the observed marginal frequencies = %0.4f\n',km)...
sprintf('k observed as proportion of maximum possible = %0.4f\n\n',ratio) ]


if k<0
  Results1 = 'Poor agreement';
elseif k>=0 && k<=0.2
  Results1 ='Slight agreement';
elseif k>0.20 && k<=0.4
  Results1 ='Fair agreement';
elseif k>0.40 && k<=0.6
  Results1 ='Moderate agreement';
elseif k>0.60 && k<=0.8
  Results1 ='Substantial agreement';
elseif k>0.80 && k<=1
  Results1 ='Perfect agreement';
end


Results1 = strcat(Results1,...
sprintf('\nVariance = %0.4f     z (k/sqrt(var)) = %0.4f    p = %0.4f' ,var,z,p));

if p<0.05
    Results2 =sprintf('Reject null hypotesis: observed agreement is not accidental');
else
    Results2 =sprintf('Accept null hypotesis: observed agreement is accidental');
end
Results = [Results,Results1];
Results = strvcat(Results,Results2);
return
end