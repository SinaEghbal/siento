function [] = emometricnew(windowlength, numberofwindows, leadin,leadout, processingPath, physioPath)
...
%     end
    ...
    numb = 1;     %% Sets the emotion number, allowing stepping through the appropriate numbers when creating the signal files, eg anger will be given ecg1, then ecg9 etc...
    count = 0;


    for h = 1 : numberofwindows,
        stnum = num2str(numb);

        for sig = 1:3 % for each window, runs through once for each signal
            if sig == 1
                sigtype = 'ecg';
            elseif sig == 2
                sigtype = 'sc';
        ...
        numb = numb + 1; % Signal number counter (emotion specific)
...
labelnames = {'Confusion';'Frustration';'Boredom';'Flow/Engagement';'Curiosity' ;'Delight';'Surprise';'Neutral'};
